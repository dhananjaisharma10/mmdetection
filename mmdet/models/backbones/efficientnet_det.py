...
class EfficientNetDet(nn.Module):
    """
    ...
    def __init__(self,
        ...
        for block_args in self._blocks_args:
            ...
            for _ in range(block_args.num_repeat - 1):
                self._blocks.append(MBConvBlock(block_args, self._global_params))
            # Out indices contain the blocks representing the end of each stage
            self._out_indices.append(len(self._blocks) - 1)
        assert len(self._out_indices) == 7
        print(self._out_indices, flush=True)
    ...
    def _check_model_name_is_valid(self, model_name, also_need_pretrained_weights=False):
        """ Validates model name. None that pretrained weights are only available for
        the first four models (efficientnet-b{i} for i in 0,1,2,3) at the moment. """
        num_models = 4 if also_need_pretrained_weights else 8
        valid_models = ['efficientnet-b'+str(i) for i in range(num_models)]
